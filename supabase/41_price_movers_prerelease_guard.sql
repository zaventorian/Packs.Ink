-- Migration 41: pre-release guard in price_movers.
--
-- Pre-release TCGPlayer prices for new sets are wildly inflated (often 10-20×
-- the post-release floor) because there are 1-2 lonely listings and no real
-- volume. When those rows leak into price_movers as a card's "previous"
-- baseline, every card in a freshly-released set shows a fake -90%+ drop the
-- moment retail goes live.
--
-- Fix: per card, drop any prices_daily row whose date is before the card's
-- set's released_at + 1 day BEFORE computing the per-window baselines. Sets
-- with NULL released_at (Promos, Extras, anything Lorcast doesn't date) get
-- fail-open behavior — no filtering, so they keep working.
--
-- Same shape as 26_price_movers_fix_prev.sql; only the underlying row
-- source changes via a new `card_release_floor` CTE + a `filtered` CTE that
-- gates `prices_daily` against the per-pid floor.
--
-- Idempotent: drops and recreates the matview.
