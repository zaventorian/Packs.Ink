...
- **The played rule was checked against the alternative and kept.** Tickets/event ranges from 12 down to under 1 across tracked stores, which looks like the rule is undercounting shops that don't run pairings through RPH. It isn't: 20,302 of 21,803 registrations (93%) carry a result, and only 300 of 3,743 events are "dark" (registrations, no results at all). The remaining ~7% is a plausible no-show rate. A store with a low count has few RPH *registrations*, not unrecorded results — and since RPH scores from its own records, that is what RPH sees too. `report_store_tiers.py`'s **Reg** and **Dark** columns are how to re-check this; don't loosen the rule to registrations without re-running them, because that trades undercounting for counting no-shows.
