...
- **`supabase/127_watchlists.sql`** — STAGED, not applied. Watchlists + items. Until it lands the Screener hides the star column and the watch bar entirely (`watchlistsUnavailable`); nothing else changes.
- **`supabase/128_market_index.sql`** — STAGED, not applied. Market indices. Contains a `drop materialized view … cascade` guard for re-runs, so it needs a human. Until it lands the "vs Mkt" column is absent and the benchmark picker hides itself after its first null fetch. **This is the slowest refresh on the site** — full history over ~3M rows — so watch the first run's timing; its `statement_timeout` is pinned at 5min.
- **`supabase/129_price_alerts.sql`** — STAGED, not applied. Alert rules + firing ledger. Until it lands the alerts modal shows its "isn't switched on yet" branch and no evaluation runs.
