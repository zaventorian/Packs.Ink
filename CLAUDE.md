...
- **Sources**: `elo_event_summary_v` (one row per event, `is_ignored` already filtered) for events + tickets, and **`elo_matches`** for the head count. The obvious choice for the second one is `elo_player_events_v`, which already reads one-row-per-(player, event) — **don't**. That view `GROUP BY`s the whole ratings table, and `sbFetchAll` re-runs it once per 1000-row page plus a `count=exact`; anon's statement timeout is 3s, so a signed-out visitor is the one who pays. `elo_matches` is an indexed straight read and carries two players per row.
- **Tickets = `num_players ?? rated_players ?? 0`**, summed. RPH doesn't always publish a headcount; the rated count is the honest fallback and both columns are already on the summary view.
...
- **RPH tier verdicts (`RPH_TIERS` / `rphTierFor`)** are the doc's published bars over "the four most recent set seasons" — Standard 25/25/250, Legendary 50/50/500 (events / unique fans / tickets). **The Tier column reports `—` for any window that isn't that one**: three sets can't clear a four-set bar and ten sets clears it trivially, so a verdict out of its window is worse than no verdict. The **Prerelease requirement is NOT scored** — it needs `kind='prerelease'` per store, which lives in `lorcana_events`, not the Elo ingest this tab reads, so a store can show Legendary here and still miss it. Say so rather than implying a full verdict.
...
- **`supabase/120_price_movers_full_catalog.sql`** — ⚠️ **APPLY THIS ONE FIRST, it's a live bug.** Drops the `>= $5 baseline` gate from `price_movers`, which was hiding 89% of the catalog from the Screener — **every Common (0 rows of 1886)**, most Uncommons (15/1413), most Rares (38/1269); 622 rows total vs 5877 in `card_prices_latest`. Reported via the feedback widget 2026-08-14 ("commons only … nothing showed up. I think the screener is hiding results"). Recreates the matview, so it's a manual SQL-editor run. **Ordering: apply BEFORE pushing the client**, because the same commit raises `synthetic_monitor.py`'s `MIN_MOVERS_ROWS` 300 → 3000 and the monitor would alert against the old 622-row matview. The home movers banners do NOT regress — they re-apply their own per-window `prior < 5` floor client-side. After applying, `select count(*) from price_movers` should read ~5,500–5,900.
