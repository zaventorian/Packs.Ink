...
- **`supabase/130_market_index_scopes.sql`** — STAGED, not applied. Rebuilds both index matviews with the 8 scopes and the coverage floor. Contains `drop materialized view … cascade` (guarded), so it needs a human. **Two steps**: paste the file, then run `select public.refresh_market_index();` separately — it creates WITH NO DATA on purpose so the initial build doesn't die on the SQL editor's own statement timeout. Until the refresh runs both matviews exist and are empty, which the client treats exactly like missing.
