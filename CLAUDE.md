...
- **`supabase/169_calendar_chattanooga_london_youth.sql`** — **HALF-APPLIED 2026-09-24.** From two
  Ravensburger OP graphics. The **Comic Con Chattanooga CCQ** half (confirm the scan candidate,
  Nov 7-8, Core Constructed, geocoded, venue = the show not the RPH store) was written live via
  REST and verified. The **DLC London Youth Division notes** half was refused by the auto-mode
  classifier and still needs a paste — re-running the whole file is safe.
