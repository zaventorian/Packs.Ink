...
- **`supabase/123_drop_prerelease_events.sql`** — drops `prerelease_events` (dead since migration 113). The precondition is DONE: `discover_prereleases.py`'s `main()` upsert path was retired 2026-08-22 (the script is analysis-only now; `discover_events.py` still imports its `classify()` and owns the daily write). Safe to run any time.
