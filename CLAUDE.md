...
3. **"Has two printings" must be decided range-independently** (2026-08-12). Deriving it from only the fetched window made the HEADLINE value change with the chart range: a C1 card whose last month saw only Top Prize foil sales read as one-printing at 1M (valuing a Prize Wall slab at the foil price) and as split at 1Y. `gradedKnownBuckets(rollupRows)` builds all-time buckets from `graded_sales_rollup`; pass it as the `knownBuckets` arg to `makeGradedSlotSeries` / `computeSalesValueHistory*` / `addUnsoldSlabsToSeries` — base series and missing-slab detection must share the SAME map or a wrong-bucket window double-counts. Both value-chart surfaces (home `CollectionPanel`, Collection-tab `GradedValueChart`) and the home graded movers' `isSplit` are wired. `computeSalesValueHistory*` also append a forward-filled point at TODAY so every range's series ends on the same date (endpoint = headline can't shift with the window's last sale date).
...
- **News feed** (`news` panel, reworked 2026-08-12) bundles the pre-release News tile, any live `EVENT_TILES` convention tile, and the Format Coconut tile — in that order (dated things first, the standing Coconut notice last, per user request). It is **pinned**: a `.home-news-feed` aside riding the Rare–Legendary movers banner as the right-hand cell of the `.rl-news-row` grid (`minmax(0,1fr) minmax(150px,32%)`) at EVERY width — the same geometry the old mobile tournament column used. The old full-width announce tiles ate half a phone screen when two conventions ran at once. `HOME_PANELS` marks it `pinned` (string = placement label in the settings editor): only its on/off toggle applies, its stored `col` is ignored, and `moveHomePanel` + the editor's `sameCol` math skip pinned keys so ▲/▼ on a panel the user moved into "Top" can't dead-swap with it. Natural heights only (`align-items:start`, list scrolls inside `max-height`) — do NOT height-lock the pair; that's what killed both `ChaseRowWithTourney` predecessors. In the movers loading/error/no-direction branches the feed renders full-width above the status message so announcements never vanish behind a data hiccup. The `announce` strip (full-width, under the search box, `repeat(auto-fit,minmax(260px,420px))`) still exists as a column target for the OTHER panels; nothing defaults there anymore.
